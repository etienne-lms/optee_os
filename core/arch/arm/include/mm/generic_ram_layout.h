 ...
 *
 * Optional directives:
 * CFG_TEE_LOAD_ADDR			Default to TEE_RAM base if not defined
 * CFG_TEE_VA_SIZE			Some platform may have specific needs
 *
 ...
 *
 * Optional directive when SDP is enabled:
 * CFG_TEE_SDP_MEM_SIZE			If CFG_TEE_SDP_MEM_BASE is not defined,
 *					CFG_TEE_SDP_MEM_SIZE sets the SDP test
 *					memory byte size.
 *
 ...
 * TA_RAM_START		TA contexts/pagestore RAM base physical base address
 * TA_RAM_SIZE		TA contexts/pagestore RAM base physical base address
 ...
 * TZSRAM_BASE		Internal secure RAM base address, required by pager.
 * TZSRAM_SIZE		Internal secure RAM byte size, required by pager.
 *
 * TEE_LOAD_ADDR	Only defined in CFG_TEE_LOAD_ADDR is defined.
 *			Otherwise we expect the platform_config.h defines it.
 * TEE_VA_SIZE		Default to a MMu pgdir size or set to CFG_TEE_VA_SIZE
 *
 * TEE_SDP_TEST_MEM_BASE Define if a SDP memory pool is required and none set.
 *			 Always defined in the inner to top (high addresses)
 ...
 *  +----------------------------------+ <---- (TA_RAM is aligned on Mmu pgdir)
 *  +----------------------------------+ <--
...
#ifdef CFG_TEE_VA_SIZE
#define TEE_VA_SIZE		CFG_TEE_VA_SIZE
#else
#define TEE_VA_SIZE		CORE_MMU_PGDIR_SIZE
#endif

#ifdef CFG_SHMEM_SIZE
#define TEE_SHMEM_SIZE		CFG_SHMEM_SIZE
#else
#define TEE_SHMEM_SIZE		SIZE_2M
#endif
