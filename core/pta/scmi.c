					  ...
					  TEE_Param params[TEE_NUM_PARAMS])
{
	...
	if (IS_ENABLED(CFG_SCMI_MSG_SMT)) {
		...
		scmi_smt_threaded_entry(channel_id);
		scmi_smt_unset_shared_buffer(channel);
...
static enum optee_scmi_ocall_reply pta_scmi_ocall(uint32_t channel_id)
{
	...
	if (!uuid_ready) {
		...
		IMSG("pta uuid [%#llx %#llx]", uuid_octet[0], uuid_octet[1]);
		uuid_ready = true;
	}
	...
	case PTA_SCMI_OCALL_PROCESS_CHANNEL:
	...
	case PTA_SCMI_OCALL_ERROR:
		break;
	default:
		DMSG("Invalid ID %u from Ocall for channel %u"PRIu32,
		     (unsigned int)agent_request, channel_id);
					...
					TEE_Param params[TEE_NUM_PARAMS])
{
	...
	while (1) {
		...
		case PTA_SCMI_OCALL_PROCESS_CHANNEL:
			continue;
		case PTA_SCMI_OCALL_CLOSE_THREAD:
			FMSG("Closing Ocall for channel %u"PRIu32, channel_id);
			return TEE_SUCCESS;
		case PTA_SCMI_OCALL_ERROR:
		default:
			DMSG("Error on Ocall for channel %u"PRIu32, channel_id);
