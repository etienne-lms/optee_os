...
#ifndef CFG_CRYPTO_RSA_NOPAD_SIGNATURE
		if (cs->mode == TEE_MODE_SIGN || cs->mode == TEE_MODE_VERIFY) {
			res = TEE_ERROR_GENERIC;
			break;
		}
#endif
		if (cs->mode == TEE_MODE_ENCRYPT || cs->mode == TEE_MODE_VERIFY) {
			res = crypto_acipher_rsanopad_encrypt(o->attr, src_data,
							      src_len, dst_data,
							      &dlen);
		} else if (cs->mode == TEE_MODE_DECRYPT || cs->mode == TEE_MODE_SIGN) {
